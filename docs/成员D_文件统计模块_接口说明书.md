...
| 页面 | 路由 | 访问角色 | 功能 |
|---|---|---|---|
| 文件管理 | `/files` | `TEACHER`、`ADMIN` | 文件查询、上传、预览、下载、删除 |
| 统计分析报表 | `/reports` | `TEACHER`、`ADMIN` | 总览卡片、状态统计、上传趋势、CSV 导出 |

文件管理页面调用 `src/api/file.ts`，预览和下载均通过携带 JWT 的 Blob 请求完成，避免直接打开受保护文件地址时丢失 `Authorization` 头。
